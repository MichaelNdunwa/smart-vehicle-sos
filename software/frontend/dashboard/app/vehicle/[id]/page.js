    ...
    <div className="max-w-[1200px] px-6 pb-10 pt-6">
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        <section className="rounded-xl bg-surface-card p-6 shadow-sm ring-1 ring-border-default">
          <h2 className="mb-4 flex items-center gap-2 text-base font-bold text-text-primary">
            <PersonIcon />
            Passengers on {vehicleId}
          </h2>
          {vehiclePassengers.length === 0 ? (
            <EmptyState
              icon={<PersonIcon />}
              text="No passengers on this vehicle."
              hint="Passengers will appear here once they board."
            />
          ) : (
            <div className="grid gap-2.5">
              {vehiclePassengers.map((passenger) => {
                const isLinked = !!passenger.tripId;
                return (
                  <article
                    key={passenger.id}
                    className="group flex items-center justify-between gap-4 rounded-lg border border-border-default p-4 transition-all duration-150 hover:border-brand-200 hover:bg-brand-50/30"
                  >
        ...
        <section className="rounded-xl bg-surface-card p-6 shadow-sm ring-1 ring-border-default">
          <h2 className="mb-4 flex items-center gap-2 text-base font-bold text-text-primary">
            <SosIcon />
            SOS alerts for {vehicleId}
          </h2>
          {vehicleAlerts.length === 0 ? (
            <EmptyState
              icon={<SosIcon />}
              text="No SOS alerts for this vehicle."
              hint="All clear."
            />
          ) : (
            <div className="grid gap-2.5">
              {vehicleAlerts.map((alert) => (
                <article
                  key={alert.id}
                  className="group relative overflow-hidden rounded-lg border border-danger-200 bg-surface-sos p-4 transition-all duration-150 hover:border-danger-300 hover:shadow-sm"
                >
                  ...
                  <div className="mt-3 grid gap-1 text-xs text-text-secondary">
                    <p>
                      ...
                      <span className="font-mono text-text-primary">
                        {alert.coordinates.lat ?? "unknown"},{" "}
                        {alert.coordinates.lng ?? "unknown"}
                      </span>
                    </p>

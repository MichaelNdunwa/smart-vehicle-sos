...
const int    PIN_LED_RED = 10; // ATmega phys pin 16 (PB2) → GPS Lock Indicator → Arduino D10
...
void setup() {
  ...
  digitalWrite(PIN_LED_RED, LOW); // RED LED off until GPS locks
...
void loop() {
  ...
  if (!panicArmed) {
    powerOffCheck();             // ← verify SIM808 is still alive
...
void processGPSData() {
  ...
  if (got) {
    digitalWrite(PIN_LED_RED, HIGH); // GPS locked
    Serial.println(F("\n>> LOCATION LOCKED"));
    ...
    digitalWrite(PIN_LED_RED, LOW);  // GPS searching
...
// ============================================================
//  SIM808 ALIVE CHECK
//  Called every GPS cycle. If the module stops responding:
//  1. LEDs go off
//  2. Auto power-on is attempted
//  3. GPS engine is restarted if module comes back
// ============================================================
void powerOffCheck() {
  sim808.println(F("AT"));
  delay(300);
  ...
  if (sim808.available()) {
    clearBuffer();
    digitalWrite(PIN_LED_GREEN, HIGH); // still alive
    return;
  }

  // Module not responding — LEDs off
  Serial.println(F("\n[WARN] SIM808 not responding. Attempting reboot..."));
  ...
  // Wait up to 10s for module to come back
  Serial.print(F("Waiting for boot"));
  unsigned long start = millis();
  while (millis() - start < 10000) {
    sim808.println(F("AT"));
    ...
    if (sim808.available()) {
      ...
      digitalWrite(PIN_LED_GREEN, HIGH); // module confirmed back
  ...
  // Still no response — warn and halt
  Serial.println();
  Serial.println(F("╔══════════════════════════════════╗"));
  Serial.println(F("║      ⚠  SIM808 NOT FOUND         ║"));
  Serial.println(F("╠══════════════════════════════════╣"));
